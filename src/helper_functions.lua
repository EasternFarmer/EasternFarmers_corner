...
function EF.FUNCS.hour_check(card)
	...
	if
		(card.ability.immutable.min_hour <= hour and hour <= card.ability.immutable.max_hour) -- normal check
...
function EF.FUNCS.year_day_check(card)
	...
	if
		(card.ability.immutable.min_yday <= yday and yday <= card.ability.immutable.max_yday) -- normal check
		or G.GAME.used_vouchers.v_EF_stopwatch -- voucher
	then
		return true
	end
