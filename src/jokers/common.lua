SMODS.Joker({
	...
	calculate = function(self, card, context)
		if
			...
			or (context.end_of_round and context.game_over == false and context.main_eval and not context.blueprint)
		then
			if
				SMODS.pseudorandom_probability(card, "EF_seed", 1, card.ability.extra.odds, "EF_seed")
				or context.forcetrigger
				or card.ability.cry_rigged -- cryptid compat
			then
				card:start_dissolve()
