...
function EF.FUNCS.UIDEF.snake_game()
    return {
        ...
        nodes = {
            {n = G.UIT.O, config = {align = "tm", func = "EF_snake_update", object = UIBox{definition=EF.FUNCS.UIDEF.snake_field(), config = {}}}},
        }}
end
...
local function snake_cell(x, y)
    ...
    local cell = {n = G.UIT.C, config = {cell_pos = {x=x,y=y}, align = "cm", minh = config.cell_minh, minw = config.cell_minw, colour = color, r=0.1, hover = true},
    nodes = {
        {n=G.UIT.T, config={ref_table = EF.vars.minigames.snake.field[y][x], ref_value = 'text', font = SMODS.Fonts.EF_OpenArrow, scale = 0.75, colour = G.C.UI.TEXT_LIGHT, shadow = true}}
    }}
    return cell
end
...
function EF.FUNCS.UIDEF.snake_field()
    ...
    config.width = (EF.vars.minigames.snake.difficulty_settings[EF.vars.minigames.snake.difficulty] or {}).width or config.width
    config.height = (EF.vars.minigames.snake.difficulty_settings[EF.vars.minigames.snake.difficulty] or {}).height or config.height
    config.speed_modifier = (EF.vars.minigames.snake.difficulty_settings[EF.vars.minigames.snake.difficulty] or {}).speed_modifier or config.speed_modifier
    ...
    local button_config = {button="EF_snake_button", align="cm", r=0.1, minw=1, minh=1, colour = G.C.BLUE, active_color = G.C.ORANGE}
    return {
        n=G.UIT.ROOT,
        ...
        nodes = {
            {n=G.UIT.C, config = {align="cm", padding = 0.5}, nodes = {
                {n=G.UIT.R, config = {align = "cm"}, nodes = {
                        {n=G.UIT.T, config={text = "Current direction: ", scale = 0.5, colour = G.C.UI.TEXT_LIGHT, shadow = true}},
                        {n=G.UIT.T, config={ref_table = EF.vars.minigames.snake.stats, ref_value = "curr_direction", scale = 0.5, colour = G.C.UI.TEXT_LIGHT, shadow = true}}
                }},
                {n=G.UIT.R, config = {align="tm"}, nodes = row_nodes},
            }},
            {n=G.UIT.C, config = {align="cm"}, nodes = { -- buttons
                {n=G.UIT.R, config = {align="cm"}, nodes = {
                    {n=G.UIT.C, config = {id="EF_snake_up", button=button_config.button, align=button_config.align, r=button_config.r, minw=button_config.minw, minh=button_config.minh, colour = button_config.colour}, nodes = {{n=G.UIT.T, config={text = "↑", font=SMODS.Fonts.EF_OpenArrow, scale = 1, colour = G.C.UI.TEXT_LIGHT, shadow = true}}}}
                }},
                {n=G.UIT.R, config = {align="cm", padding = 0.1}, nodes = {
                    {n=G.UIT.C, config = {id="EF_snake_left", button=button_config.button, align=button_config.align, r=button_config.r, minw=button_config.minw, minh=button_config.minh, colour = button_config.colour}, nodes = {{n=G.UIT.T, config={text = "←", font=SMODS.Fonts.EF_OpenArrow, scale = 0.8, colour = G.C.UI.TEXT_LIGHT}}}},
                    {n=G.UIT.C, config = {id="EF_snake_down", button=button_config.button, align=button_config.align, r=button_config.r, minw=button_config.minw, minh=button_config.minh, colour = button_config.colour}, nodes = {{n=G.UIT.T, config={text = "↓", font=SMODS.Fonts.EF_OpenArrow, scale = 0.8, colour = G.C.UI.TEXT_LIGHT}}}},
                    {n=G.UIT.C, config = {id="EF_snake_right", button=button_config.button, align=button_config.align, r=button_config.r, minw=button_config.minw, minh=button_config.minh, colour = button_config.colour}, nodes = {{n=G.UIT.T, config={text = "→", font=SMODS.Fonts.EF_OpenArrow, scale = 0.8, colour = G.C.UI.TEXT_LIGHT}}}},
                }}
            }},
        },
    }
...
function EF.FUNCS.UIDEF.snake_info()
    return {
        ...
        config = {r = 0.1, minw = 15, minh = 8, align = "cm", colour = G.C.BLACK},
        nodes = {
            {n = G.UIT.R, config = {align = "tm"}, nodes = SMODS.localize_box(loc_parse_string("{C:white}Snake"), {scale = 3})},
            {n = G.UIT.R, config = {align = "tm"}, nodes = {{n=G.UIT.T, config={text="", scale = 0.7, colour = G.C.WHITE, shadow = true}}}, {scale = 1.6}},
            {n = G.UIT.R, config = {align = "tm"}, nodes = SMODS.localize_box(loc_parse_string("{C:white}Use arrow keys or the blue"), {scale = 1.6})},
            {n = G.UIT.R, config = {align = "tm"}, nodes = SMODS.localize_box(loc_parse_string("{C:white}buttons to change moving direction"), {scale = 1.6})},
            {n = G.UIT.R, config = {align = "tm"}, nodes = {{n=G.UIT.T, config={text="", scale = 0.7, colour = G.C.WHITE, shadow = true}}}, {scale = 1.6}},
        }
    }
end
...
function EF.FUNCS.snake.snake_collision(direction)
    ...
    if n_x < 1 or n_x > config.width or
        n_y < 1 or n_y > config.height then
        return true
    end
...
function G.FUNCS.EF_snake_update(e)
    ...
    if EF.vars.minigames.snake.clock % EF.vars.minigames.snake.target_fps == 0 then
        ...
        EF.vars.minigames.snake.target_fps = math.max(config.minimum_speed, math.ceil(EF.vars.minigames.snake.target_fps))
    else
        return
    end
...
function G.FUNCS.EF_snake_button(e) -- also called from overrides.lua
    local new_dir_table = {EF_snake_up = "up",EF_snake_down = "down", EF_snake_left = "left", EF_snake_right="right"}
...
function EF.FUNCS.UIDEF.snake_score_tab()
    ...
    return {
        ...
        config = {r = 0.1, minw = 15, minh = 8, align = "cm", colour = G.C.BLACK},
        nodes = {
            {n = G.UIT.R, config = {align = "tm"}, nodes = {{n=G.UIT.T, config={text="Snake", scale = 1, colour = G.C.WHITE, shadow = true}},}},
            {n = G.UIT.R, config = {align = "tm"}, nodes = {{n=G.UIT.T, config={text="Score: "..score.." / "..max_score, scale = 0.7, colour = G.C.WHITE, shadow = true}},}},
            {n = G.UIT.R, config = {align = "tm"}, nodes = SMODS.localize_box(loc_parse_string("{C:white}Reward nr. "..chosen..": "..prize.text), {scale = 2})},
        }}
end
...
function EF.FUNCS.UIDEF.snake_reward_info()
    ...
    local nodes = {
        {n = G.UIT.R, config = {align = "tm"}, nodes = {{n=G.UIT.T, config={
            text=difficulty == "hard" and "Snake (hard)" or "Snake (easy)", scale = 1, colour = G.C.WHITE, shadow = true
        }}}}
    }
    ...
    for i,v in ipairs(reward_list[difficulty]) do
        ...
        table.insert(nodes, {n=G.UIT.R, config={align = "cm", padding = 0.03}, nodes=SMODS.localize_box(loc_parse_string("{C:white}"..message), {scale = 2})})
    end
