...
local function minesweeper_cell(config, x, y)
	return {
		n = G.UIT.C,
		config = {
			button = "EF_minesweeper_cell",
			orginal_config = config,
			cell_pos = { x = x, y = y },
			align = "cm",
			...
			r = 0.1,
			tooltip = {
				title = "Click to uncover",
				text = {
					"(No flags available)",
					"Current bombs: " .. math.floor(config.width * config.height / config.mines),
				},
			},
			juice = true,
			hover = true,
		...
		nodes = {
			{
				n = G.UIT.T,
				config = {
					ref_table = EF.vars.minigames.minesweeper.FIELD_TABLE[y][x],
					ref_value = "text",
					scale = 0.75,
					colour = G.C.WHITE,
					shadow = true,
...
local function minesweeper_row(config, y)
	...
	return {
		n = G.UIT.R,
		config = { align = "cm", minh = config.cell_minh, minw = config.cell_minw, padding = config.cell_padding },
		nodes = nodes,
	}
end
...
function EF.FUNCS.UIDEF.minesweeper_field()
	...
	return {
		...
		config = {
			r = 0.1,
			minw = config.cell_minw * config.width + 2,
			minh = config.cell_minh * config.height + 2,
			align = "cm",
			colour = G.C.BLACK,
		},
		nodes = nodes,
...
function G.FUNCS.EF_minesweeper_cell(e)
	local function placemine(config)
		...
		if
			x ~= e.config.cell_pos.x
			and y ~= e.config.cell_pos.y
			and not EF.vars.minigames.minesweeper.FIELD_TABLE[y][x].bomb
		then
				...
				nodes = {
					{
						n = G.UIT.T,
						config = {
							text = "Click on a tile to reveal what is underneath.",
							scale = 0.7,
							colour = G.C.WHITE,
							shadow = true,
						},
				...
				nodes = {
					{
						n = G.UIT.T,
						config = {
							text = "There can either be a number or a bomb.",
							scale = 0.7,
							colour = G.C.WHITE,
							shadow = true,
						},
				...
				nodes = {
					{
						n = G.UIT.T,
						config = {
							text = "A number tells you have many bombs are ",
							scale = 0.7,
							colour = G.C.WHITE,
							shadow = true,
						},
				...
				nodes = {
					{
						n = G.UIT.T,
						config = {
							text = "adjacent to the tile with the number.",
							scale = 0.7,
							colour = G.C.WHITE,
							shadow = true,
						},
				...
				nodes = {
					{
						n = G.UIT.T,
						config = {
							text = "Clicking on a bomb ends the minigame.",
							scale = 0.7,
							colour = G.C.WHITE,
							shadow = true,
						},
				...
				nodes = {
					{
						n = G.UIT.T,
						config = {
							text = "The more points you get the better reward will be.",
							scale = 0.7,
							colour = G.C.WHITE,
							shadow = true,
						},
				...
				nodes = {
					{
						n = G.UIT.T,
						config = {
							text = "Small amount of points may result in a bad effect.",
							scale = 0.7,
							colour = G.C.WHITE,
							shadow = true,
						},
...
function EF.FUNCS.UIDEF.minesweeper_score_tab()
	...
	local max_score = game_config.width * game_config.height
		- math.floor(game_config.width * game_config.height / game_config.mines)
	return {
		n = G.UIT.ROOT,
		config = { r = 0.1, minw = 15, minh = 8, align = "cm", colour = G.C.BLACK },
		nodes = {
				...
				nodes = {
					{
						n = G.UIT.T,
						config = {
							text = "Score: " .. EF.vars.minigames.minesweeper.score .. " / " .. max_score,
							scale = 0.7,
							colour = G.C.WHITE,
							shadow = true,
						},
			...
			},
			{
				...
				nodes = SMODS.localize_box(
					loc_parse_string("{C:white}Reward nr. " .. v[1] .. ": " .. v[2]),
					{ scale = 2 }
				),
			},
		},
...
function EF.FUNCS.minesweeper.give_reward_set_text()
	...
	local game_config = EF.vars.minigames.minesweeper.config
	local max_points = game_config.width * game_config.height
		- math.floor(game_config.width * game_config.height / game_config.mines)
	...
	EF.vars.minigames.minesweeper.reward_table =
		{ reward_chosen, EF.vars.minigames.minesweeper.config.reward_list[difficulty][reward_chosen] }
end
